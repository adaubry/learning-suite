...
// Bloc 5.2 (É3.1–É3.2) : adaptateurs minces vers S4.SessionService — un point
...
export async function retenterAction(cycleId: string) {
  const userId = await requireUserId();
  await session.resolveOutcome(userId, cycleId, "retenter");
  redirect("/");
}
...
export async function revelerAction(cycleId: string, _prevState: unknown, _formData: FormData) {
  const userId = await requireUserId();
  return session.resolveOutcome(userId, cycleId, "reveler");
}
...
export async function terminerAction(cycleId: string) {
  const userId = await requireUserId();
  await session.terminerSessionTemporaire(userId, cycleId);
  redirect("/");
}
