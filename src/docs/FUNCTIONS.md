...
| U25 | **LectureView**            | Assemblage U2 (FocusShell) + U3 (MarkdownViewer) sur le contenu de la section + `[Je suis prêt, je blurte]` + `[Abandonner]`. Deux contextes : lecture initiale et relecture ciblée — même texte, même composant, seule la prop `relecture` (booléen, cosmétique) change le badge affiché ; **v0.7, 2026-07-15** : le diff (U16.`DiffList`) en vis-à-vis retiré de l'écran. `[Je suis prêt, je blurte]` ouvre un compte à rebours de 30s (`[Passer maintenant]` en échappatoire, `[Abandonner]` toujours dispo) avant d'appeler `S4.terminerLecture` — **v0.6, 2026-07-15** : retour sur le comportement structurel sans minuteur de la v0.4. **v0.9, 2026-07-16** : pendant la lecture (hors compte à rebours), marge de notes libres à droite (`AnnotatedCourse`, écran large uniquement) — clic sur un bloc du cours (ancrage `data-md-start` de U3, comme U5) pose une note à la hauteur exacte de ce bloc, en marge, jamais par-dessus le texte ; la note fait partie du même flux de page que le texte donc suit le scroll nativement (pas de `position: fixed`) ; état React local uniquement, aucune persistance ni appel serveur. |
...
| 0.10    | 2026-07-16 | `AnnotatedCourse` (v0.9) : retour humain — poser une note faisait défiler toute la page horizontalement (`autoFocus` natif du textarea déclenche le scroll-into-view du navigateur), donnant l'impression que le cours « saute » à gauche pendant que la note apparaît coupée à droite. Focus posé manuellement (`ref` stable + `.focus({ preventScroll: true })`) au lieu de `autoFocus`. `overflow-x-hidden` ajouté sur `<body>` (app/layout.tsx, toutes les routes) en garde-fou : aucune interaction ne doit jamais provoquer de défilement horizontal de la page. |
| 0.9     | 2026-07-16 | `ReadingNotes` (v0.8, ancrage par clic brut, `position: fixed`) remplacé par `AnnotatedCourse` : retour humain — les notes ne suivaient pas le scroll et chevauchaient le texte. Nouvel ancrage sur le bloc `data-md-start` cliqué (même mécanisme que U5 AnomalyPanel), note positionnée en `absolute` dans le même conteneur `relative` que le texte (suit le scroll nativement, plus de `fixed`), marge strictement à droite (`left-full`), jamais par-dessus la colonne de cours. Toujours purement client, aucune persistance. |
| 0.8     | 2026-07-16 | U25 LectureView gagne `ReadingNotes` : marge de notes libres à droite pendant la lecture (écran large, hors compte à rebours) — clic dans la marge pose une note à la hauteur du clic, texte libre, jamais persisté (état React local, aucun service/DB touché). Purement client, aucun changement de schéma ni de `S4`. |
