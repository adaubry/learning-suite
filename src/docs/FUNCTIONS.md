...
| U25 | **LectureView**            | Nouveau (REVAMP v2, 2026-07-15) : assemblage U2 (FocusShell) + U3 (MarkdownViewer) sur le contenu de la section + `[Je suis prêt, je blurte]` + `[Abandonner]`. Deux contextes : lecture initiale (texte seul) et relecture ciblée (texte + U16.`DiffList` de la correction précédente en vis-à-vis) — même composant, prop de contexte. |
