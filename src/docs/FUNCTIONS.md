...
| U25 | **LectureView**            | Assemblage U2 (FocusShell) + U3 (MarkdownViewer) sur le contenu de la section + `[Je suis prêt, je blurte]` + `[Abandonner]`. Deux contextes : lecture initiale (texte seul) et relecture ciblée (texte + U16.`DiffList` de la correction précédente en vis-à-vis) — même composant, prop de contexte. `[Je suis prêt, je blurte]` ouvre un compte à rebours de 30s (`[Passer maintenant]` en échappatoire, `[Abandonner]` toujours dispo) avant d'appeler `S4.terminerLecture` — **v0.6, 2026-07-15** : retour sur le comportement structurel sans minuteur de la v0.4. |
...
| É3.1 Lecture (étude uniquement, 2 occurrences) | U2 + U25 (U3, + U16.`DiffList` en relecture ciblée) |
