...
// Accent — deep indigo (OKLCH H=285), sitting in the one unclaimed gap
// between the categorical Blue (H=255) and Purple (H=320) hues so it never
// gets confused with a status color. Chosen for the product: a law
// student's daily study console reads better on institutional ink-blue
// than on a "brand" color borrowed from either status hue. Used only for
// primary actions, focus/selection rings, and accent text/links — the rest
// of the UI stays the neutral theme's grayscale (Restrained strategy, one
// deliberate accent).
const ACCENT_LIGHT = 'oklch(0.42 0.17 285)';
const ACCENT_DARK = 'oklch(0.78 0.13 285)';
...
export const neutralTheme = defineTheme({
  ...
  tokens: {
    ...
    '--color-accent-muted': ['oklch(0.94 0.025 285)', 'oklch(0.42 0.17 285 / 26%)'],
    ...
    '--color-text-accent':    [ACCENT_LIGHT, ACCENT_DARK],
    '--color-on-dark':    '#ffffff',
    '--color-on-light':   '#171717',
    // Contrast: indigo accent is dark enough (L) / light enough (D) for
    // white-on-fill / near-black-on-fill respectively — same pattern as
    // success/error below.
    ...
    // Icon
    '--color-icon-accent':    [ACCENT_LIGHT, ACCENT_DARK],
    ...
    '--shadow-inset-hover': 'inset 0px 0px 0px 2px oklch(0.42 0.17 285 / 30%)',
    '--shadow-inset-selected': 'inset 0px 0px 0px 2px oklch(0.42 0.17 285 / 50%)',
  ...
  components: {
    ...
    // =========================================================================
    // SideNavItem — the scrollable items container has no gap between rows
    // by default (only inline padding), so items visually touch. Margin
    // between rows, mobile and desktop share the same component.
    // =========================================================================
    'side-nav-item': {
      base: {
